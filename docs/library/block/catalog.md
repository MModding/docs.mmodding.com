...
| `UpsideSensitiveBlock`         | An upside sensitive block is a block that will have an `influence` block state property depending on the detected above block. So yeah, it works like a grass block with snow above.                              |
